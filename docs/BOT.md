...
正文使用 Markdown。**按信源分板块组装**，不要再压成「主线 1/2/3」硬归类。

> **选题与篇幅（2026-09-07 定调）：**
> - 全天大约 **3–5 条**真正值得展开的重点即可；由总管自判权重，**没有就不要勉强**。
> - 板块（权威行业 / 开源 / 模型 / 大V与实践）**有料才写，无料整节跳过**，禁止空占位与「本窗无高信号」。
> - 单条深度对齐 **2026-09-04 / 09-06** 那种「可深挖、话不多」——保留机制/证据/工程含义；禁止超长白话说明书，也禁止压回 5k 短讯。
> - 脚本下限 2500 仍是拒短讯底线；合格日目标观感约 **1.0–1.3 万** 紧凑字，宁缺毋滥。
...
目标读者：**AI 研究学习者** 与 **开发从业者**。每条重点用克制篇幅讲清三件事：讲什么、为何重要、对谁有用/值不值得深读；再给 FACT/官方自报/待核与可点链接。必要术语一句话解释。**禁止**纯罗列，也**禁止**把一条写成说明书。
...
四大内容板块**按需出现**（无料整节省略；不要为齐全而凑）：

| 顺序 | 板块标题（建议） | 主要材料来自 | 写什么 |
|---|---|---|---|
| 1 | `## 权威行业动态` | **研小宝** | 大厂官博/研究页、高信号预印本；有料时按 OpenAI→Anthropic→Google→Meta AI→Nvidia→其他官博→arXiv 分列，无则跳过 |
| 2 | `## 热门 AI 开源项目跟踪` | **G小宝** | Release、热仓、关键 org |
| 3 | `## 业界模型相关动态` | **模型小宝** | 窗口更新 vs 仅热度；量化/模型卡 |
| 4 | `## AI 大V 观点与业界热门实践` | **X小宝** | Following、**Today's News**、公开高信号实践/观点 |

有展开的条目可不强制再套「重点关注/次要动态」两级；需要扫尾时再用次要。板块内允许表格、时间线。中文为主；专有名词保留原文。
...
- 专才素材经总管交叉核实后，只把当日 3–5 条高权重写深；其余可丢或一句带过。
