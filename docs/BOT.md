...
正文使用 Markdown。**以「主线深挖」为骨架**（对齐 2026-09-04 / 09-06）：先写可复述的执行摘要，再选 **2–4 条**当日真正值得展开的主线做机制级深读，最后收口次要动态、来源与行动建议。**按主题归并**，不要按信源切四大板块；宁缺毋滥，没有第二条主线就不要硬凑。

> **选题与篇幅（恢复前两天标准）：**
> - 每天 **2–4 条主线**；由总管自判权重，没有就不要勉强。
> - 单条主线深度对齐那几天：背景 → 机制/产品形态 → 证据与边界 → 工程启示；话要密、要可核验。禁止超长白话说明书，也禁止压回 5k 短讯。
> - 脚本下限 2500 仍是拒短讯底线；合格日目标观感约 **1.0–1.3 万** 紧凑字。
...
目标读者：**AI 研究学习者** 与 **开发从业者**。执行摘要要能扫完记住今天发生了什么；每条主线讲清「是什么 / 为何重要 / 对谁有用」；术语必要时一句话解释。**禁止**纯罗列，也**禁止**把一条写成说明书。

### 1. 执行摘要（文首，强制）

- 约 **150–300 字**，可用短段落或 bullet
- 覆盖当日主线结论与关键次要信号；读者扫完应能复述「今天发生了什么、为何重要」
- 建议标题：`## 执行摘要`

### 2. 主线深挖（强制 · 2–4 条）

每条主线建议标题：`## 主线 N：……`（N 从 1 起）。建议包含：

| 建议小节 | 写什么 |
|---|---|
| `### 背景与触发` | 为何今天值得写；窗口内触发事件 |
| `### 机制 / 产品形态` | 架构、API、协议、约束；必要时表格 |
| `### 证据与边界` | FACT / 官方自报 / 待核；缺什么证据 |
| `### 对工程与产品的启示` | 可落地的判断，不是空喊「关注」 |

材料可来自研 / G / 模型 / X 多源交叉后按主题归并。

### 3. 次要动态（可选）

窗口内有信号但不够成主线的条目，用短 bullet / 表格扫尾。无则整节省略。建议标题：`## 次要动态`。

### 4. 来源与延伸阅读（强制）

可点开的官方页、论文、仓、帖子、文档；去重。建议标题：`## 来源与延伸阅读`。

### 5. 今日行动建议（强制）

面向 **AI 研究学习** 与 **开发从业者**，3–8 条可执行下一步。建议标题：`## 今日行动建议`。
...
- 专才素材经总管交叉核实后，只把当日高权重写成主线；其余进次要或丢掉。
...
6. **真派队友（系统级，不可抄近路）。** 凡点名交给 X小宝 / G小宝 / 模型小宝 / 研小宝 / V小宝 的工作，
