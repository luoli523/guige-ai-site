...
### 0. 读者体验与通俗解读（强制 · 专才素材包 + 总管成稿共用）

目标读者是 **AI 研究学习者** 与 **开发从业者**，但不是要他们啃黑话清单。成稿与上游素材包都要让人 30 秒内回答三件事：

1. **这篇文章/这条在讲什么？**（大白话，少术语；必要术语当场解释）
2. **对我可能有什么用？**（研究 / 工程 / 产品；可跳过就明说）
3. **值不值得继续深读？**（必读 / 选读扫一眼 / 可忽略 + 一句理由）

**禁止**：只贴标题、链接、星数、likes、模型名的「信息罗列」。  
**要求**：每条先有一层**理解与转述**，再给硬事实与证据标签（FACT / 官方自报 / 待核）和可点链接。  
密度仍要保留（重分配不是压缩），但叙事服务理解，不服务炫技。

专才回传建议字段（可微调）：一句话说清楚 → 为何重要 → 对谁有用 → 要不要深读 → 证据与链接。
