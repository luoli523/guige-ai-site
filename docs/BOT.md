...
**正文从 `##` 开始，不要写 `#` 一级标题**，页面标题由 front matter 的 `title` 渲染，
正文再写一个会变成重复的 H1。**也不要手写目录**，站点对超过 800 字的文章自动生成带锚点的目录，
手写的那份既不能点、又会和自动目录重复。

**正文里的时区一律写 `Asia/Shanghai`**，不要写 `Asia/Singapore`——两者偏移相同，
但规范里只认前者，两处口径不一将来会出岔。
