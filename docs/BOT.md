...
不要为了「人话」牺牲精确度：数字、日期、版本号一个都不能含糊，
每个关键数字和结论都要能顺着链接找到出处。**要的是又准又好读，不是二选一。**
...
`sources` 字段如实列出当天实际用到的渠道短名（如 `github.com`、`huggingface.co`、`arxiv.org`）。同一事件多源合并；二手传闻找不到一手来源就丢弃。
...
正文**不再使用** `FACT / 官方自报 / 待核` 之类的标签。可信度靠写法体现：

- 关键数字、日期、版本号、结论，都要能顺着句中或「来源与延伸阅读」里的链接找到一手出处。  
- 厂商自己的评测数字，句子里直接写明「官方给出的数字是……」，不要写成已被独立验证。  
- **不确定的信息不要写成确定事实。** 找不到一手来源的二手传闻、缺原始帖的演示，直接删掉，不要靠标签兜底。  
