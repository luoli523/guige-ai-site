...
>
> **FACT** [OpenAI Computer use 文档](https://…)。原生 `computer` 工具仍作备选。

### 允许的表达

类比、对照表、时间线、「一句话结论」、小标题里带判断（`## 主线 1：OpenAI 承认监控快不够用了`
好于 `## 主线 1：OpenAI 同日双发`）。

### 不要做的事

不要为了「人话」牺牲精确度：数字、日期、版本号一个都不能含糊，
`FACT / 官方自报 / 待核` 三级标注照旧全部保留。**要的是又准又好读，不是二选一。**
...
正文使用 Markdown。**第一节固定是 `## 一分钟速览`**（见[渲染契约](#渲染契约不可改动)第 5 条），其后**以「主线深挖」为骨架**（对齐 2026-09-04 / 09-06）：先写可复述的执行摘要，再选 **2–4 条**当日真正值得展开的主线做机制级深读，最后收口次要动态、来源与行动建议。**按主题归并**，不要按信源切四大板块；宁缺毋滥，没有第二条主线就不要硬凑。
