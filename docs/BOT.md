...
| 2 | `## 热门 AI 开源项目跟踪` | **G小宝** | Release、热仓、关键 org 动态 |
| 3 | `## 业界模型相关动态` | **模型小宝** | Trending / 新模型卡 / 量化与数据集等；区分「窗口内更新」与「仅热度」 |
| 4 | `## AI 大V 观点与业界热门实践` | **X小宝** | Following + 公开高信号实践/观点；演示完整度标待核 |

#### 权威行业动态 · 厂商分列（强制）

`## 权威行业动态` **不要**只用「重点关注/次要动态」两桶糊弄。有料时按下列顺序选用 `###` 分列：

1. `### OpenAI`
2. `### Anthropic`
3. `### Google`（含 DeepMind 官博/研究页）
4. `### Meta AI`
5. `### Nvidia`
6. `### 其他官博`（Microsoft Research、其他实验室等）
7. `### arXiv cs.AI/CL/LG`

**无料就跳过该 `###`，不要写「本窗无高信号」占位。** 栏目内可再拆 `####` 展开头条；量化/模型卡仍归「业界模型相关动态」，勿把 HF 部署包装塞进 Nvidia 官博栏充数。
